...
async function getManageCredentials() {
    ...
    try {
        const response = await fetch(`${url}/data/manageCredentials`, {
            method: "GET", headers: {
...
async function exportManageCredentials() {
    try {
        toggleRefreshButton(true);
        const response = await fetch(`${url}/data/manageCredentials/export`)
        const data = await response.json();
        ...
        a.href = exportUrl;
        a.download = 'manageCredentials.json';
...
async function fetchAdditionalData(username) {
    toggleRefreshButton(true);
    try {
        const response = await fetch(`${url}/data/manageCredentials/${username}`)
...
async function handleEditFormSubmit(event) {
    ...
    const tableName = editForm.getAttribute('data-tableName');
    const oldUsername = editForm.getAttribute('data-username');
    const newUsername = editForm.querySelector('input[name="username"]').value;
    formData.username = newUsername;

    try {
        const response = await fetch(`${url}/data/${tableName}/${oldUsername}`, {
...
function populateEditForm(username, userData) {
    ...
    for (let key in userData) {
        if (userData.hasOwnProperty(key)) {
            ...
            if (key !== 'Cleartext-Password' && key !== 'MAC') {
...
function createAddButton() {
    ...
    addButton.textContent = '+'; // Текст кнопки
...
function openDeleteModal(username, tableName) {
    ...
    // Добавляем информацию о пользователе и таблице в попап
...
async function handleDeleteButtonClick(username, tableName) {
    ...
    document.getElementById('confirmDeleteButton').addEventListener('click', async (event) => {
        event.preventDefault();
        try {
            const response = await fetch(`${url}/data/${tableName}/${username}`, {
    ...
    const closeDeleteManageCredentialsPopupButton = document.getElementById('closeDeleteManageCredentialsPopupButton');
    closeDeleteManageCredentialsPopupButton.addEventListener('click', () => {
        popup.style.display = "none"; // Закрываем попап при нажатии на крестик
    });
}
