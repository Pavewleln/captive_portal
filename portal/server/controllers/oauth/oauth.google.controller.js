...
export const OauthGoogle = async (req, res) => {
    ...
    try {
        ...
        const request = {
            ...
            attributes: [['User-Name', email], ['User-Password', payload.sub], ['Calling-Station-Id', mac]]
        };
        await sendRadiusRequest(request);
        const response = await receiveRadiusResponse();

        if (response.code !== 'Access-Accept') return res.redirect(process.env.WEB_URL + "login");
